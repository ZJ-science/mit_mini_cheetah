...
void vectornav_handler(void* userData, VnUartPacket* packet,
 ...
 //   std::cout << "-----quat=" << quat.c[0] << "\t" << quat.c[1] << "\t" << quat.c[2] << "\t" << quat.c[3] << std::endl;
//    std::cout << "-----omeg=" << omega.c[0] <<"\t" << omega.c[1] << "\t" << omega.c[2] << std::endl;
//     std::cout << "-----aaaa=" << a.c[0] <<"\t" << a.c[1] << "\t" << a.c[2] << std::endl;
  //liu
 ...
 if(zj::flag_imu_read)
  {
    zj::flag_imu_read = false;
    printf("imu已经读到～\n");
  }
  vectornav_lcm_data.read_flag = true;
