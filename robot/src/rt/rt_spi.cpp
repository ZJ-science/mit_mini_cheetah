...
const float max_torque[3] = {17.f, 17.f, 17.f};  // TODO CHECK WITH BEN   26.0f
...
const float knee_side_sign[4] = {-1.f, 1.f, -1.f, 1.f};//.06429f

// only used for actual robot
const float abad_offset[4] = {0.f, 0.f, 0.f, 0.f};
const float hip_offset[4] = {M_PI / 2.f, -M_PI / 2.f, -M_PI *1.5f, M_PI * 1.5f};
...
void spi_to_spine(spi_command_t *cmd, spine_cmd_t *spine_cmd, int leg_0) {
  for (int i = 0; i < 2; i++) {
    ...
    spine_cmd->q_des_hip[i] =
        ...
        hip_offset[i + leg_0];
    if(cmd->q_des_knee[i + leg_0]<0.79)
	cmd->q_des_knee[i + leg_0]=0.79;
 ...
 /*
  ...
  {
    printf("set data=");
    printf("qdes: %.3f\t\%.3f\t", cmd->q_des_abad[0], cmd->q_des_abad[1]);
    printf("%.3f\t\%.3f\t", cmd->q_des_hip[0], cmd->q_des_hip[1]);  
    printf("%.3f\t\%.3f\r\n", cmd->q_des_knee[0], cmd->q_des_knee[1]);
    
   // printf("qd_des: %.3f\t\%.3f\t", spine_cmd->qd_des_abad[0], spine_cmd->qd_des_abad[1]);
   // printf("%.3f\t\%.3f\t", spine_cmd->qd_des_hip[0], spine_cmd->qd_des_hip[1]);  
   // printf("%.3f\t\%.3f\r\n", spine_cmd->qd_des_knee[0], spine_cmd->qd_des_knee[1]);
    
   // printf("kp: %.3f\t\%.3f\t", spine_cmd->kp_abad[0], spine_cmd->kp_abad[1]);
   // printf("kd: %.3f\t\%.3f\t\n", spine_cmd->kd_abad[0], spine_cmd->kp_abad[1]);
    
   // printf("tau_abad_ff: %.3f\t\%.3f\t\n",  spine_cmd->tau_abad_ff[0], spine_cmd->tau_abad_ff[1]);
    
   // printf("flags: %d\t%d\t", spine_cmd->flags[0], spine_cmd->flags[1]);
  }
  */
  //liu
}
...
void spine_to_spi(spi_data_t *data, spine_data_t *spine_data, int leg_0) {
  ...
  else
  {
    ...
    if(leg_0==0){
    printf("feed data=");
    printf("q_abad: %.3f\t\%.3f\t", spine_data->q_abad[0], spine_data->q_abad[1]);
    printf("%.3f\t\%.3f\t", spine_data->q_hip[0], spine_data->q_hip[1]);  
    printf("%.3f\t\%.3f\t\n", spine_data->q_knee[0], spine_data->q_knee[1]);
    
    printf("\t qd: %.3f\t\%.3f\t", spine_data->qd_abad[0], spine_data->qd_abad[1]);
    printf("%.3f\t\%.3f\t", spine_data->qd_hip[0], spine_data->qd_hip[1]);  
    printf("%.3f\t\%.3f\r\n", spine_data->qd_knee[0], spine_data->qd_knee[1]);
  //liu
    }
    if(leg_0 == 0){
      printf("F data=");
      printf("q_abad: %.3f\t\%.3f\t", data->q_abad[0+leg_0], data->q_abad[1+leg_0]);
      printf("%.3f\t\%.3f\t", data->q_hip[0+leg_0], data->q_hip[1+leg_0]);  
      printf("%.3f\t\%.3f\t\n", data->q_knee[0+leg_0], data->q_knee[1+leg_0]);
      
      printf("qd: %.3f\t\%.3f\t", data->qd_abad[0+leg_0], data->qd_abad[1+leg_0]);
      printf("%.3f\t\%.3f\t", data->qd_hip[0+leg_0], data->qd_hip[1+leg_0]);  
      printf("%.3f\t\%.3f\r\n", data->qd_knee[0+leg_0], data->qd_knee[1+leg_0]);
    }
    else if(leg_0 == 2)
    {
      printf("H data=");
...
void spi_driver_run() {
  /*
  static std::ofstream mylog[4] = {std::ofstream("/home/billchent/log/rf.csv"), 
				   std::ofstream("/home/billchent/log/lf.csv"),
				   std::ofstream("/home/billchent/log/rh.csv"),
				   std::ofstream("/home/billchent/log/lh.csv")
  };*/
  ...
  /*
  for (int i = 0; i < 4; i++){
    mylog[i] << spi_command_drv.q_des_abad[i] << ",";
    mylog[i] << spi_command_drv.q_des_hip[i] << ",";
    mylog[i] << spi_command_drv.q_des_knee[i] << ",";
    mylog[i] << ",";
    mylog[i] << spi_command_drv.qd_des_abad[i] << ",";
    mylog[i] << spi_command_drv.qd_des_hip[i] << ",";
    mylog[i] << spi_command_drv.qd_des_knee[i] << ",";
    mylog[i] << ",";
    mylog[i] << spi_data_drv.q_abad[i] << ",";
    mylog[i] << spi_data_drv.q_hip[i] << ",";
    mylog[i] << spi_data_drv.q_knee[i] << ",";
    mylog[i] << ",";
    mylog[i] << spi_data_drv.qd_abad[i] << ",";
    mylog[i] << spi_data_drv.qd_hip[i] << ",";
    mylog[i] << spi_data_drv.qd_knee[i] << ",";
    mylog[i] << std::endl;
  }*/
