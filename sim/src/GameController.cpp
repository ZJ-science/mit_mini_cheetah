...
void GameController::updateGamepadCommand(GamepadCommand &gamepadCommand) {
  if (_qGamepad) {
    ...
    Vec2<float>(-_qGamepad->axisLeftX(), -_qGamepad->axisLeftY()*1.5) ; // x velocity  in range(-0.8 1.5)

    if(gamepadCommand.leftStickAnalog[1]<-0.8)
      gamepadCommand.leftStickAnalog[1]=-0.8;
