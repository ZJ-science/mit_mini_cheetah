...
#include "rt/rt_interface_lcm.h"  //zj
//#include "vectornav_lcmt.hpp"
//bool readed_imu = false;
...
ControlFSM<T>::ControlFSM(Quadruped<T>* _quadruped,
  ...
  statesList.down = new FSM_State_Down<T>(&data);
...
void ControlFSM<T>::runFSM() {
  zj_Handler makesure_read_imu;
  makesure_read_imu.readed_imu = false;
  //lcm::LCM zj_lcm;
  // Check the robot state for safe operation
  if(currentState->stateName==FSM_StateName::LOCOMOTION) 
      operatingMode = safetyPreCheck();
 
  get_main_control_settings(&main_control_settings);
  if(data._desiredStateCommand->returnBackGamepad()->a||main_control_settings.mode == 1)
    data.controlParameters->control_mode = K_STAND_UP;
  if(data._desiredStateCommand->returnBackGamepad()->y||main_control_settings.mode == 4)
    data.controlParameters->control_mode = 4;
  if(data._desiredStateCommand->returnBackGamepad()->b||main_control_settings.mode == 6)
    data.controlParameters->control_mode = 6;
  if(data._desiredStateCommand->returnBackGamepad()->x||main_control_settings.mode == 10)
    data.controlParameters->control_mode = 10;
  if(data._desiredStateCommand->returnBackGamepad()->leftTriggerAnalog||main_control_settings.mode==102) 
    data.controlParameters->control_mode = 102;
  ...
  if (operatingMode != FSM_OperatingMode::ESTOP) {
	...
	/*if(currentState == statesList.locomotion)
	{
	  zj_lcm.subscribe("hw_vectornav",&zj_Handler::handleMessage,&makesure_read_imu);
	  if(makesure_read_imu.readed_imu)
	  {;}
	  else
	    currentState = statesList.down;
	}*/
    ...
    if(data._desiredStateCommand->returnBackGamepad()->a||main_control_settings.mode ==101) 
      data.controlParameters->control_mode = K_TC;
    if(data.controlParameters->control_mode == K_TC)
    {
      printf("强制退出estop\n");
      printf("zj-set\n");
      operatingMode = FSM_OperatingMode::NORMAL;
...
FSM_State<T>* ControlFSM<T>::getNextState(FSM_StateName stateName) {
  ...
  switch (stateName) {
    ...
    case FSM_StateName::Down:
      return statesList.down;
...
void zj_Handler::handleMessage(const lcm::ReceiveBuffer* rbuf, const std::string& chan, const vectornav_lcmt* msg)
{
  std::cout<<rbuf<<chan<<msg<<std::endl;

  readed_imu = msg->read_flag;
}
