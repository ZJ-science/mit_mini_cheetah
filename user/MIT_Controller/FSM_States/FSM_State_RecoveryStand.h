...
class FSM_State_RecoveryStand : public FSM_State<T> {
 ...
 private:
  ...
  // 0.5 kHz
  const int rollover_ramp_iter = 150*2;//150
  const int rollover_settle_iter = 150*2;

  //const int fold_ramp_iter = 500;
  //const int fold_settle_iter = 500;
  const int fold_ramp_iter = 400*2;
  const int fold_settle_iter = 700*2;

  const int standup_ramp_iter = 250*2;
  const int standup_settle_iter = 250*3;
