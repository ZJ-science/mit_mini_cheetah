...
ConvexMPCLocomotion::ConvexMPCLocomotion() :
  ...
  walking2(int(horizonLength*1.2), Vec4<int>(0,6,6,0), Vec4<int>(6,6,6,6), "short_tort"),
  //pacing(int(horizonLength*2.0), Vec4<int>(0,5,10,15),Vec4<int>(5,5,5,5),"long_trot")
  pacing(int(horizonLength*1.4), Vec4<int>(0,7,7,0),Vec4<int>(7,7,7,7),"long_trot")
{
...
//  debugSphere->radius = 1;
  ...
  {
    rpy_int[1] += 0.003*(stateCommand->data.stateDes[4] /*-hw_i->state_estimator->se_ground_pitch*/ - seResult.rpy[1])/v_robot[0];
  }
  if(fabs(v_robot[1]) > 0.01)
  {
    rpy_int[0] += 0.003*(stateCommand->data.stateDes[3] /*-hw_i->state_estimator->se_ground_pitch*/ - seResult.rpy[0])/v_robot[1];
  }
 //0.003  0.003
  ...
  for(int i = 0; i < 4; i++)
  {
    ...
    //if(firstSwing[i]) {
      footSwingTrajectories[i].setHeight(main_control_settings.foot_height); ////////////////////////
...
//{1.25,1.25,10,2,2,50,0,0,0.3,1.5,1.5,0.2}
    ...
    if(current_gait == 4)   //站立步态
    {
      float trajInitial[12] = {(float)stateCommand->data.stateDes[3],
                               ...
                               (float)0.22/*fsm->main_control_settings.p_des[2]*/,
                               0,0,0,0,0,0};
