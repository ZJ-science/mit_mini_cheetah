...
void DesiredStateCommand<T>::convertToStateCommands() {
  ...
  if(main_control_settings.mode == 100) //zj
  {
...
//      }
  ...
  else//rc
  {
    // Forward linear velocity
  data.stateDes(6) =
      deadband(main_control_settings.v_des[0], minVelX, maxVelX);
...
//      }
  // Lateral linear velocity
  data.stateDes(7) =
      deadband(main_control_settings.v_des[1], minVelY, maxVelY);
  ...
  // Yaw turn rate
  data.stateDes(11) =
      deadband(main_control_settings.omega_des[2], minTurnRate, maxTurnRate);
